...
p = argparse.ArgumentParser(
        description="Injection in Batches",
...
output_base = REPO_ROOT / "output"/ f"{args.flavor}" /"test"/"injection_files"
...
def simulate_batch(settings):
    ...
    # Configuration
    config.run.storage_prefix = str(output_base)
    ...
    print(f"[Worker with seed={id}] starting injection")
    
    prometheus = Prometheus()
    prometheus.inject()
    
    print(f"[Simulation saved in {str(output_base)} with file_id #{id}]")
    del prometheus
    gc.collect()
    return (id,n_events)
...
if __name__ == "__main__":
    ...
    # Checks if this seed has been done before and changes it
    check=0
    run_dir = output_base / f"{check}_LI_output.h5"
