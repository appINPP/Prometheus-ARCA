...
class Prometheus(object):
    ...
    def propagate(self, capture=False, event_ids=None):
        ...
        if nevents is not None:
            logger.info("Propagating %d events", nevents)

        # -------------------------
        # photon propagator setup
        # -------------------------
        if pp_name == "olympus":
        ...
        if event_ids is not None:
            # Yield (event_id, event_data) tuples for specified IDs
            iterator = ((eid, self.injection[eid]) for eid in event_ids)
        else:
            # Yield (index, event_data) enumeration for all events
        ...
        show_progress = bool(nevents is not None and nevents > threshold)

        if show_progress:
            try:
                from tqdm import tqdm
                iterator = tqdm(iterator, total=nevents, desc="Propagating events")
            except ImportError:
                pass
        ...
        try:
            ...
            for idx, injection_event in iterator:
                ...
                if nevents is not None and event_ids is None and idx == nevents:
                    break

                for final_state in injection_event.final_states:
                    if show_progress and hasattr(iterator, "set_description"):
                        try:
                            iterator.set_description(f"Propagating event {idx} state {final_state}")
                        except Exception:
                            pass
    ...
    def propagate(self, capture=False, event_ids=None):
        ...
        try:
            ...
            for idx, injection_event in iterator:
                ...
                for final_state in injection_event.final_states:
                    ...
                    # NOTE: DB marking logic removed from here entirely!
        ...
        try:
            ...
            # -------------------------
            if event_ids is None:
                base_iter = enumerate(self.injection)
            else:
                base_iter = ((i, self.injection[i]) for i in event_ids)

            if show_progress:
                iterator = tqdm(base_iter, total=nevents)
            else:
                iterator = base_iter
            ...
            for idx, injection_event in iterator:
                ...
                for final_state in injection_event.final_states:
                    ...
                    try:
                        ...
                        raise
                    if event_ids is not None and mark_done is not None:mark_done(config.run.db_path, idx) # idx corresponds to the event_id
    ...
    def construct_output(self):
        ...
        # Get the IDs we want
        ids = getattr(self, "_last_event_ids", None)
        
        # Slice the event list directly
        if ids is not None:
            # We pluck the specific event objects into a standard Python list
            target_events = [self.injection.events[i] for i in ids]
        else:
            target_events = self.injection.events

        # Serialization
        # NOTE: set_serialization_index usually expects the injection object. 
        # If it fails with a list, use self.injection but know indexing might be off.
        set_serialization_index(self.injection) 
        
        json_config = json.dumps(config.to_dict())
        
        # We pass the target_events list to the serializer
        test_arr = serialize_particles_to_awkward(self.detector, target_events)
        
        # Building the Awkward Array
        # We must slice the mc_truth awkward array to match our IDs
        full_mc_truth = self.injection.to_awkward()
        sliced_mc_truth = full_mc_truth[ids] if ids is not None else full_mc_truth

        if test_arr is not None:
            outarr = ak.Array(
                {
                    "mc_truth": sliced_mc_truth,
                    config.photon_propagator.photon_field_name: test_arr,
                }
            )
        else:
            outarr = ak.Array({"mc_truth": sliced_mc_truth})
    ...
    def construct_output(self):
        ...
        from .utils.serialization import (
            ...
            set_serialization_index,
        )

        # Get event IDs stored during propagate()
        ids = getattr(self, "_last_event_ids", None)
        ...
        # Mark events as DONE in the SQLite database AFTER file write succeeds
        db_path = getattr(config.run, "db_path", None)
        if ids is not None and db_path and mark_done is not None:
            try:
                from .job_db import mark_many_done
                mark_many_done(db_path, ids)
            except ImportError:
                # Fallback to loop if mark_many_done isn't available
                for eid in ids:
                    mark_done(db_path, eid)
