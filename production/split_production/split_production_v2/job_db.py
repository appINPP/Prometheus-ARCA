...
def mark_many_done(db_path, event_ids):
    """Mark a list of event IDs as DONE in a single transaction."""
    if not event_ids:
        return
    conn = sqlite3.connect(db_path)
    conn.execute("PRAGMA busy_timeout = 30000")
    c = conn.cursor()
    c.executemany("UPDATE events SET status='DONE' WHERE id=?", [(i,) for i in event_ids])
    conn.commit()
    conn.close()
