...
## ✩ Output to GNN
To prepare the raw simulation files (photons) for the training we use two scrips subsequently: photonstopulses.py and datatognndata.py. Both scripts handle multiple parquet files inside directories and their subdirectories, using multiprocessing. Before running, check out the parameters of the function `get_safe_worker_count` to make sure it aligns with your RAM use expectations.

**photonstopulses.py** creates new parquet files containing the information on hit positions, directions, dom/pmt ids and electronics. **datatognndata.py** takes the photons and pulses parquets and creates a new unified parquet file with the features of our choice. Those unified parquets are then ready to be condensed and transferred for further analysis.

## ✩ Validation
To validate the simulation output we may use:
+ **scatterplot.py**, to create scatterplots with data from multiple files. This is based on Melina's plotting codes.
...
## ✩ Caution
+ datatognndata.py might be case sensitive to what values are written and whether they're nested, double nested and so on. It requires some caution if ToT isn't included.
