...
To create the injection files you run injection.py with the same arguments used in batches_production.py. This also creates a folder named by the event type and a subfolder named "injection files" to store them in.

To run the simulations you should first initialize the event distributor for your spcecific file, found by the arguments of event type (--flavor) and (--id) the identification number used for the injection file. Once that's set you can run the propagation.py code to create simulations while using arguments such as --flavor, --file_id, --workers, --events_per_worker and --energy with the inputs high or low depending on whether on what energy events you want to propagate. Parquet files will be saved inside the event type folder inside the subfolder named "simulation_files". Good luck.
